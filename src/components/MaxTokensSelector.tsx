...
const MaxTokensSelector: React.FC<MaxTokensSelectorProps> = ({ maxTokens, onChangeMaxTokens }) => {
  ...
  const currentOptionLabel =
    tokenOptions.find(option => option.value === maxTokens)?.label || '8K tokens';
  ...
  return (
    <div className="max-tokens-selector">
      ...
      <p className="mt-2 text-sm" style={{ color: theme === 'light' ? '#6b7280' : '#9ca3af' }}>
