...
const LOCAL_STORAGE_SYSTEM_MESSAGE_ENABLED_KEY =
	"next-lm-chat-system-message-enabled";
const LOCAL_STORAGE_SYSTEM_MESSAGE_CONTENT_KEY =
	"next-lm-chat-system-message-content";
...
const ChatContainer: React.FC = () => {
	...
	const [systemMessageEnabled, setSystemMessageEnabled] =
	...
	useEffect(() => {
		const fetchModels = async () => {
			try {
				...
				if (
					savedModelId &&
					modelData.some((model) => model.id === savedModelId)
				) {
					setSelectedModel(savedModelId);
				} else if (
					modelData.some((model) => model.id === APP_CONFIG.defaultModel)
				) {
									...
									if (part.startsWith("data: ")) {
										...
										try {
											...
											try {
												...
												console.debug(
													"Data after character replacement:",
													data.substring(0, 300),
												);
											} catch (escapeError) {
												console.error(
													"Error during character unescaping:",
													escapeError,
												);
											...
											try {
												...
												try {
													...
													console.log(
														"Successfully parsed JSON after sanitizing escape sequences",
													);
												} catch {
													...
													try {
														...
														console.log(
															"Successfully parsed JSON using reviver function",
														);
													} catch {
											...
											if (content.includes("<think>")) {
												...
												fullContent += "<think>";
												console.log(
													"Detected thinking mode, displaying in thinking box",
												);
