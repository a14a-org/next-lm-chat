...
const MessageList: React.FC<MessageListProps> = ({
	...
	const lastContentLengthRef = useRef(
	...
	const {
		debouncedFn: debouncedSetNewContent,
		cancel: cancelDebouncedSetNewContent,
	} = useDebounce(
		(value: boolean) => {
	...
	}, [handleUserInitiatedContentUpdate]);
	...
	useEffect(() => {
		...
		if (!autoScrollEnabled && messages.length > 0) {
			...
			if (
				...
				(currentLastMessage &&
					currentLastMessage.content.length > lastContentLengthRef.current)
			) {
